(() => {
    ...
    try{
      ...
      try{
        ...
        for (let i=0;i<trs.length;i++){
          ...
          const ket = tr.querySelector('td:nth-child(7) input, .ket'); if (ket) ket.value = it.keterangan||'';
        }
      }catch{}
    }catch{}
  })();
  ...
  function parseExcelRows(rows){
    ...
    for (let i=hIdx+1; i<rows.length; i++){
      ...
      const no = r[0]; const lokasi=r[1]; const hardware=r[2]; const sn=r[3];
      const panduan=r[4]; const kondisi=(String(r[5]||'').toLowerCase().includes('tidak')? 'tidak' : (String(r[5]||'').toLowerCase().includes('baik')? 'baik' : ''));
      const keterangan=r[6];
      if (no==null && !lokasi && !hardware && !sn && !keterangan) continue;
      items.push({ no, lokasi, hardware, sn, panduan, kondisi, keterangan });
    }
  ...
  function applyImportedData(data){
    try{
      ...
      for(let i=0;i<rows.length;i++){
        ...
        const ketEl = tr.querySelector('.ket') || tr.querySelector('td:nth-child(7) input'); if (ketEl) ketEl.value = it.keterangan||'';
        // Kondisi
  ...
  btnSubmit.addEventListener('click',()=>{
    const items = Array.from(tbody.querySelectorAll('tr')).map((tr,i)=>{
      ...
      if (isStatic){
        ...
        const ket = get('td:nth-child(7) input');
  ...
  function initSignature(canvas, clearBtn, fileInput, onDrawn){
    ...
    fileInput?.addEventListener('change',()=>{ const f=fileInput.files&&fileInput.files[0]; if(!f) return; const r=new FileReader(); r.onload=()=>{ const img=new Image(); img.onload=()=>{ ctx.clearRect(0,0,canvas.width,canvas.height); ctx.drawImage(img,0,0,canvas.width,canvas.height); onDrawn(true) }; img.src=r.result }; r.readAsDataURL(f) })
    return ctx;
  }
  ...
  function applyExistingSignatureIfReady(){
    ...
    try{
      ...
      if (sigObj.ts){ const img = new Image(); img.onload=()=>{ ctxTs.clearRect(0,0,ttdCanvasTs.width,ttdCanvasTs.height); ctxTs.drawImage(img,0,0,ttdCanvasTs.width,ttdCanvasTs.height); ttdTsDrawn=true; }; img.src = sigObj.ts; }
      if (sigObj.store){ const img2 = new Image(); img2.onload=()=>{ ctxStore.clearRect(0,0,ttdCanvasStore.width,ttdCanvasStore.height); ctxStore.drawImage(img2,0,0,ttdCanvasStore.width,ttdCanvasStore.height); ttdStoreDrawn=true; }; img2.src = sigObj.store; }
